function showData(){
	Amazon_MonitoringOverview_Category(getParameter())
}

function Amazon_MonitoringOverview_Category(v){
	showLoading();
	...
	$.ajax({
		url:path+'/sku/Amazon_MonitoringOverview_Category.htm',
		type:"POST",
		data:{
			top:v._top,
			...
			if (data.status == '1') {
				...
				if(data.data.length>0){
					...
					$.each(data.data,function(index,value){
		                ...
		                <td style="text-align: left;color: #0066c0;"><a href="${path}/sku_buyBoxOffer.htm?asin=${value.asin}&sku=${value.ourCode}" target="_blank" class="blue">${value.sku_productTitle}</a></td>
		                <td style="text-align: left;">${value.categoryName}</td>
		                <td>${value.now_rank}</td>
		                <td>${value.yestday_rank}</td>
		                <td>
